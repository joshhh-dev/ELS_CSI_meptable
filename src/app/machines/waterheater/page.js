  ...
  useEffect(() => {
    const fetchWaterHeaters = async () => {
      try {
        ...
        toast.error("Failed to load water heater machines ❌");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="p-6">
      <h1 className="text-3xl font-extrabold mb-8 text-gray-800 tracking-wide">
        Water Heater Machines
      </h1>

      {waterHeaters.length === 0 ? (
        <p className="text-gray-500 text-lg">No water heater machines found.</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {waterHeaters.map((machine) => (
            <div
              key={machine.id}
              ...
              onClick={() => router.push(`/machines/waterheater/${machine.id}`)}
            >
