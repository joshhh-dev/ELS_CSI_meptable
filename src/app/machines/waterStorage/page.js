...
export default function WaterStoragePage() {
    ...
    useEffect(() => {
        const fetchWaterStorages = async () => {
            try {
                ...
                console.error("Error fetching water storages:", error);
                toast.error("Failed to load water storage units ❌");
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <div className="p-6">
            ...
            {waterStorages.length === 0 ? (
                    <p className="text-gray-500 text-lg">No water storage units found.</p>
                  ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                      {waterStorages.map((machine) => (
                        <div
                          key={machine.id}
                          ...
                          onClick={() => router.push(`/machines/waterStorage/${machine.id}`)}
                        >
