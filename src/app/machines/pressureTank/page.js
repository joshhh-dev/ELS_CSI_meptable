...
export default function PressureTankPage() {
    const [pressureTanks, setPressureTanks] = useState([]); 
    ...
    useEffect(() => {
        const fetchPressureTanks = async () => {
            try {
                ...
                console.error("Error fetching pressure tanks:", error);
                toast.error("Failed to load pressure tanks ❌");
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <div className="p-6">
            <h1 className="text-3xl font-extrabold mb-8 text-gray-800 tracking-wide">
                Pressure Tanks units
            </h1>

            {pressureTanks.length === 0 ? (
                    <p className="text-gray-500 text-lg">No pressure tanks found.</p>
                  ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                      {pressureTanks.map((machine) => (
                        <div
                          key={machine.id}
                          ...
                          onClick={() => router.push(`/machines/pressureTank/${machine.id}`)}
                        >
                          ...
                          <button
                            ...
                            aria-label={`Add ${machine.model} to cart`}
                          >
